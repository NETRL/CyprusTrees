...
class UserEventsService
{
    public function forUser(int $userId, ?Carbon $now = null): array
    {
        ...
        return collect()
            ->merge($this->plantingForUser($userId, $now))
            ->merge($this->maintenanceForUser($userId, $now))
            ->sortBy(function ($e) {
    ...
    private function plantingForUser(int $userId, Carbon $now): Collection
    {
        $items = PlantingEvent::query()
            ->with(['campaign:id,name,sponsor', 'neighborhood:id,name'])
        ...
        return $items->map(function (PlantingEvent $e) use ($now) {
            ...
            $start = $e->started_at ? Carbon::parse($e->started_at) : Carbon::parse($e->created_at);
            ...
            $progressText = is_null($target) ? "{$treesCount} trees" : "{$treesCount}/{$target} trees";
            ...
            return [
                ...
                'detailsUrl' => route('plantingEvents.index', ['search' => $e->planting_id]),
                'actions' => $actions,
    ...
    private function maintenanceForUser(int $userId, Carbon $now): Collection
    {
        ...
        return $items->map(function (MaintenanceEvent $m) use ($now) {
            ...
            $start = $m->performed_at ? Carbon::parse($m->performed_at) : Carbon::parse($m->created_at);
            // completion moment
            ...
            return [
                ...
                'detailsUrl' => route('maintenanceEvents.index', ['search' => $m->event_id]),
                'actions' => ['complete'],
    ...
    private function formatRange(Carbon $start, ?Carbon $end): string
    {
        ...
        if ($start->isSameDay($end)) return $start->format('M j, H:i') . '–' . $end->format('H:i');
